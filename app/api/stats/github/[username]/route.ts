...
export const GET = withAuth(async (request: NextRequest, { params }: { params: { username: string } }) => {
  try {
    ...
    const reposRes = await fetch(`https://api.github.com/users/${params.username}/repos?per_page=100&sort=updated`, {
      headers: githubToken ? { Authorization: `Bearer ${githubToken}` } : {}
    })
    if (!reposRes.ok) {
